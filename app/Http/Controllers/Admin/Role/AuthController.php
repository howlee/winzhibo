...
use App\Models\Local\Admin\AdRole;
use App\Models\Local\Admin\AdRoleAccount;
...
class AuthController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $account = AdminAccount::query()->where("email", $account)->first();
        ...
        $pw = $account->password;
        //判断是否登录
        if ($pw != AdminAccount::shaPassword($salt, $password)) {
            return back()->with(["error" => "账户或密码错误"]);
        }

        $token = AdminAccount::generateToken();
        $account->token = $token;
        if ($remember == 1) {
    ...
    {
        $accounts = AdminAccount::query()->orderBy('created_at', 'desc')->get();
    ...
    public function put(Request $request)
    {
        ...
        if ($request->has('id')) {
            $account = AdminAccount::query()->find($request->input('id', 0));
            if (empty($account)) {
            ...
            $account = AdminAccount::query()->where('account', $request->input('account', ''))->first();
            if (isset($account)) {
                return back()->with(['error' => '账号名已经存在'])->withInput(['password']);
            }
            $account = AdminAccount::query()->where('email', $request->input('email', ''))->first();
            if (isset($account)) {
    ...
    public function delete(Request $request)
    {
        ...
        $account = AdminAccount::query()->find($id);
        if (!isset($account)) {
    ...
    {
        session()->forget(AdminAccount::QT_ADMIN_AUTH_SESSION);
        setcookie(AdminAccount::QT_ADMIN_AUTH_TOKEN, '', time() - 3600, '/', 'qiutantiyu.cc');
