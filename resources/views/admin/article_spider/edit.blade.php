...
@section('content')
            ...
            <div class="col-lg-12">
                <form class="form" method="post" action="/admin/article/save/">
                    ...
                    <div class="input-group form-group">
                        ...
                        <input type="text"
                               name="title"
                               value="{{ session('title',isset($article)?$article->title:$f_article->title) }}"
                               class="form-control"
                               placeholder="标题"
                               required autofocus>
                        <span class="input-group-addon">{{isset($article) ? mb_strlen($article->title) : mb_strlen($f_article->title)}}字</span>
                    </div>
                    <div class="input-group form-group">
                    ...
                    <div class="input-group form-group col-lg-3">
                        ...
                        <select style="width: 200px;" name="type" class="form-control" id="type">
    ...
    <div style="float: right;width: 50%;padding-left: 100px">
        <strong>原文</strong>
        <a target="_blank" href="{{$f_article['from_url']}}">点击去原文</a>
        <h1>{{$f_article['title']}}</h1>
        <div>
            {!! $f_article->getContent() !!}
        </div>
    </div>
@endsection
...
@section('js')
    ...
    {!! (isset($article) && strlen($article->getContent())>0)?$article->getContent():$f_article->getContent() !!}
    </script>
    <!-- 实例化编辑器 -->
        ...
        function formVerify(form) {//验证
            ...
            if (type != 12) {//世界杯类型取消限制
                if ($.trim(form.title.value).length < 11 || $.trim(form.title.value).length > 31) {
                    toastr.error('标题必须不少于10字符，不能多于31字符');
                    return false;
                }
